...
bullet_fire_img = pygame.transform.rotate(fireball_img, 180)
...
enelvl4_img = pygame.image.load("assets/002-cat-1.png").convert_alpha()
...
class Player:
    ...
    def draw(self):
        WIN.blit(self.img, (self.x, self.y))
class Enemy:
    def __init__(self, lvl, x=WIDTH/2, y=-50):
...
def display_power_ups(power_ups, player, selected_bullet):
    ...
    WIN.blit(ice_label, (425 + 30, HEIGHT - 42))
    #RAINBOW
    if player.bullet_type == "rainbow" or len(power_ups["rainbow"]) > 0:
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and len(clip) < bulmax and player.bullet_type == "normal":
                    bullet =      Bullet(player.x + 8, player.y, "normal")
                    clip.append(bullet)
        ...
        if key_pre[pygame.K_SPACE] and len(clip) < bulmax and player.bullet_type == "rainbow":
            bullet = Bullet(player.x + 8, player.y, "rainbow")
            clip.append(bullet)
